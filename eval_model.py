...
def run_experiment(output_dir, model_name, arch_type,
    ...
    if all(os.path.exists(x) for x in enc_pcs) and (all(os.path.exists(x) for x in dec_pcs)):
        ...
        with ExitStack() as stack:
            ...
            command.append("--N_levels")
            if isinstance(N_levels,int):
                command.append(str(N_levels))
            else:
                for channels in N_levels:
                    command.append(str(channels))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--N_levels",help="Number of levels in the invertible core.", type=int, nargs="+", default=2)
